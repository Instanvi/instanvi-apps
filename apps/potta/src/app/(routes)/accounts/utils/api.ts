...
import { IAccountPayload, IAccountUpdatePayload } from './validations';

export const accountsApi = {
  getOhada: async (name: string) => {
    const result = await axios.get(`/ohada-accounts/tree-by-name/${name}`);
    console.log(result);
    return result.data;
  },
  ...
  create: async (data: IAccountPayload) => {
  ...
  getAll: async (filter: Record<string, any> = {}) => {
    ...
    const result = await axios.post(`/accounts/filter?${queryParams}`);
    console.log(result);
    return result.data;
  },
  ...
  update: async (account_id: string, data: IAccountUpdatePayload) => {
