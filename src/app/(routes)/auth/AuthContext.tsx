  ...
  useEffect(() => {
    const initAuth = async () => {
      ...
      try {
        ...
        // Check URL for encrypted payload (from Super App redirect)
        const url = new URL(window.location.href);
        const encryptedPayload = url.searchParams.get('payload');
        const legacyToken = url.searchParams.get('token');

        if (encryptedPayload || legacyToken) {
          devLog('Found authentication data in URL');
          ...
          try {
            ...
            if (encryptedPayload) {
              const payload = EncryptionUtils.decrypt<AuthPayload>(encryptedPayload);
              if (payload) {
                decryptedToken = payload.token || null;
                organizationId = payload.organizationId || null;
                devLog('Decrypted secure payload', { organizationId });
              }
            } else if (legacyToken) {
              // Fallback to legacy decryption if configured
              decryptedToken = await decryptToken(legacyToken, config.encryptionSecret || "legacy-secret");
            }

            if (!decryptedToken) {
              throw new Error('Could not extract valid token from URL');
            }
            ...
            devLog('Successfully authenticated with URL data');
          } catch (err) {
            devLog('Failed to process authentication data', err);
            setError(AUTH_ERRORS.TOKEN_INVALID);
            clearStoredTokens();
            setIsLoading(false);
            return;
